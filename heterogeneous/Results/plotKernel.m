...
if min(phi(rspan(1):0.01:rspan(2))) ~= max(phi(rspan(1):0.01:rspan(2)))
    bounds = [rspan(1) rspan(2) min(phi(rspan(1):0.01:rspan(2))) max(phi(rspan(1):0.01:rspan(2)))];  % [left right bottom top] bounds
else
    bounds = [rspan(1) rspan(2) min(phi(rspan(1):0.01:rspan(2)))-1 min(phi(rspan(1):0.01:rspan(2)))+1];
end
