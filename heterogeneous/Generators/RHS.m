function dotX = RHS(t,X,Xprime,d,N,phi)
...
Nprime = length(Xprime)/d; 
Xprime = reshape(Xprime,d,Nprime); 
...
f_phi = zeros(d,N);
for i = 1:N
    r = Xprime-repmat(X(:,i),1,Nprime); % [x(1)-x(i), x(2)-x(i), ..., x(N)-x(i)]; d x N
    ...
    f_phi_i = (1/Nprime)*r*interactKer; % d x 1
